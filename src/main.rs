...
pub fn main() {
    ...
    let mut gui = Gui::new(&dev, &pass, width, height);
    ...
    'running: loop {
        ...
        let io = gui.ctx.io_mut();
        io.mouse_pos[0] = events.mouse_state().x() as f32;
        io.mouse_pos[1] = events.mouse_state().y() as f32;
        io.mouse_down[0] = events.mouse_state().left();
        io.mouse_down[1] = events.mouse_state().right();
        io.mouse_down[2] = events.mouse_state().middle();
