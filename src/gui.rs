...
impl Gui {
    ...
    pub fn new(dev: &Dev, pass: &Pass, width: u32, height: u32) -> Self {
        ...
        let bindings = im::DrawVert::get_set_layout_bindings();
        let set_layout = Pipeline::create_set_layout(&dev.device, &bindings);
    ...
    fn render(&mut self, res: &mut Frameres) {
        ...
        if res.descriptors.gui_sets.is_empty() {
            res.descriptors.gui_sets = res.descriptors.allocate(&[self.set_layout]);
