...
pub struct ModelPipelineCache {
    sets: HashMap<usize, Vec<vk::DescriptorSet>>,
    pool: vk::DescriptorPool,
    pub device: Rc<ash::Device>,
}
impl ModelPipelineCache {
...
impl Drop for ModelPipelineCache {
    fn drop(&mut self) {
...
impl ModelPipeline {
    ...
    pub fn get_cache(&mut self, frame_index: usize) -> &mut ModelPipelineCache {
        while frame_index >= self.caches.len() {
            self.caches.push(ModelPipelineCache::new(&self.device));
        }
    ...
    pub fn bind_trs(&self, set: vk::DescriptorSet) {
    ...
    /// We do not know whether descriptor sets are allocated together and stored in a vector
    /// or they are allocated one by one, therefore we just expect one descriptor set here.
    pub fn bind_view_projection(&self, set: vk::DescriptorSet) {
        let writes = [vk::WriteDescriptorSet::builder()
            .dst_set(set)
            .dst_binding(1u32)
            .dst_array_element(0)
