...
pipewriter_macro!("crates/shader/main");
...
impl PipelineMain {
    fn bind_impl(&self, frame: &mut Frame, model: &Model, camera_node: Handle<Node>) {
            ...
            .get(&(self.get_set_layouts()[1], camera_node))
        {
            ...
            let sets = frame.res.descriptors.allocate(&[self.get_set_layouts()[1]]);
    ...
    fn get_model_sets(
            ...
            .contains_key(&(self.set_layouts[0], node_handle))
        {
            ...
            let sets = frame.res.descriptors.allocate(&[self.set_layouts[0]]);
    ...
    fn get_material_sets(
            ...
            .contains_key(&(self.set_layouts[2], material_handle))
        {
            ...
            let sets = frame.res.descriptors.allocate(&[self.set_layouts[2]]);
